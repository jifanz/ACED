...
for mode in ['train', 'test']:
    ...
    for data_name in ['cifar', 'svhn', 'fashion', 'mnist']:
        ...
        if mode == 'train':
            plt.axhline(y=max(max(active_accuracy), max(passive_accuracy)), color=(.4, .4, .4), linestyle='--', linewidth=2)
        ...
        for algo, name in zip(algos, algo_names):
            if algo is not None:
                ...
                plt.plot(baseline_schedule, accum_fn(baseline_accuracy), marker='o', ms=5, label=curve_name,
                         linewidth=3)
            else:
                ax = plt.axes()
        ...
        plt.plot(active_schedule, accum_fn(active_accuracy), marker='o', ms=5, label='ACED', linewidth=3)
